...
trait Tables extends MySQLDBImpl{
  ...
  lazy val schema: profile.SchemaDescription = Array(TmChannel.schema, TmDiamodPrice.schema, TmDiamondAccount.schema, TPaymentOrder.schema, TSequence.schema, TTradeOrder.schema).reduceLeft(_ ++ _)
  ...
  case class TmChannelRow(id: Int, code: String, status: Boolean, name: String, gmtCreate: java.sql.Timestamp, gmtUpdate: java.sql.Timestamp)
  /** GetResult implicit for fetching TmChannelRow objects using plain SQL queries */
  implicit def GetResultTmChannelRow(implicit e0: GR[Int], e1: GR[String], e2: GR[Boolean], e3: GR[java.sql.Timestamp]): GR[TmChannelRow] = GR{
    prs => import prs._
    TmChannelRow.tupled((<<[Int], <<[String], <<[Boolean], <<[String], <<[java.sql.Timestamp], <<[java.sql.Timestamp]))
  }
  ...
  class TmChannel(_tableTag: Tag) extends profile.api.Table[TmChannelRow](_tableTag, Some("account"), "tm_channel") {
    def * = (id, code, status, name, gmtCreate, gmtUpdate) <> (TmChannelRow.tupled, TmChannelRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(code), Rep.Some(status), Rep.Some(name), Rep.Some(gmtCreate), Rep.Some(gmtUpdate)).shaped.<>({r=>import r._; _1.map(_=> TmChannelRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
   ...
   *  @param tradeVoucherNo Database column trade_voucher_no SqlType(VARCHAR), Length(64,true)
   *  @param memberId Database column member_id SqlType(BIGINT)
   *  @param channelId Database column channel_id SqlType(INT)
   *  @param amount Database column amount SqlType(DECIMAL)
   *  @param payStatus Database column pay_status SqlType(INT)
   *  @param gmtCreate Database column gmt_create SqlType(TIMESTAMP)
   *  @param gmtUpdate Database column gmt_update SqlType(TIMESTAMP) */
  case class TPaymentOrderRow(paymentVoucherNo: String, tradeVoucherNo: String, memberId: Long, channelId: Int, amount: scala.math.BigDecimal, payStatus: Int, gmtCreate: java.sql.Timestamp, gmtUpdate: java.sql.Timestamp)
  /** GetResult implicit for fetching TPaymentOrderRow objects using plain SQL queries */
  implicit def GetResultTPaymentOrderRow(implicit e0: GR[String], e1: GR[Long], e2: GR[Int], e3: GR[scala.math.BigDecimal], e4: GR[java.sql.Timestamp]): GR[TPaymentOrderRow] = GR{
    prs => import prs._
    TPaymentOrderRow.tupled((<<[String], <<[String], <<[Long], <<[Int], <<[scala.math.BigDecimal], <<[Int], <<[java.sql.Timestamp], <<[java.sql.Timestamp]))
  }
  ...
  class TPaymentOrder(_tableTag: Tag) extends profile.api.Table[TPaymentOrderRow](_tableTag, Some("account"), "t_payment_order") {
    def * = (paymentVoucherNo, tradeVoucherNo, memberId, channelId, amount, payStatus, gmtCreate, gmtUpdate) <> (TPaymentOrderRow.tupled, TPaymentOrderRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(paymentVoucherNo), Rep.Some(tradeVoucherNo), Rep.Some(memberId), Rep.Some(channelId), Rep.Some(amount), Rep.Some(payStatus), Rep.Some(gmtCreate), Rep.Some(gmtUpdate)).shaped.<>({r=>import r._; _1.map(_=> TPaymentOrderRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7.get, _8.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Database column trade_voucher_no SqlType(VARCHAR), Length(64,true) */
    val tradeVoucherNo: Rep[String] = column[String]("trade_voucher_no", O.Length(64,varying=true))
    /** Database column member_id SqlType(BIGINT) */
    val memberId: Rep[Long] = column[Long]("member_id")
    /** Database column channel_id SqlType(INT) */
    val channelId: Rep[Int] = column[Int]("channel_id")
  ...
  /** Entity class storing rows of table TTradeOrder
   *  @param tradeVoucherNo Database column trade_voucher_no SqlType(VARCHAR), PrimaryKey, Length(64,true)
   ...
   *  @param diamandAmount Database column diamand_amount SqlType(INT)
   *  @param amount Database column amount SqlType(DECIMAL)
   *  @param tradeStatus Database column trade_status SqlType(INT)
   *  @param gmtCreate Database column gmt_create SqlType(TIMESTAMP)
   *  @param gmtUpdate Database column gmt_update SqlType(TIMESTAMP) */
  case class TTradeOrderRow(tradeVoucherNo: String, paymentVoucherNo: String, memberId: Long, tradeType: Int, diamandAmount: Int, amount: scala.math.BigDecimal, tradeStatus: Int, gmtCreate: java.sql.Timestamp, gmtUpdate: java.sql.Timestamp)
  /** GetResult implicit for fetching TTradeOrderRow objects using plain SQL queries */
  implicit def GetResultTTradeOrderRow(implicit e0: GR[String], e1: GR[Long], e2: GR[Int], e3: GR[scala.math.BigDecimal], e4: GR[java.sql.Timestamp]): GR[TTradeOrderRow] = GR{
    prs => import prs._
    TTradeOrderRow.tupled((<<[String], <<[String], <<[Long], <<[Int], <<[Int], <<[scala.math.BigDecimal], <<[Int], <<[java.sql.Timestamp], <<[java.sql.Timestamp]))
  }
  /** Table description of table t_trade_order. Objects of this class serve as prototypes for rows in queries. */
  class TTradeOrder(_tableTag: Tag) extends profile.api.Table[TTradeOrderRow](_tableTag, Some("account"), "t_trade_order") {
    def * = (tradeVoucherNo, paymentVoucherNo, memberId, tradeType, diamandAmount, amount, tradeStatus, gmtCreate, gmtUpdate) <> (TTradeOrderRow.tupled, TTradeOrderRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(tradeVoucherNo), Rep.Some(paymentVoucherNo), Rep.Some(memberId), Rep.Some(tradeType), Rep.Some(diamandAmount), Rep.Some(amount), Rep.Some(tradeStatus), Rep.Some(gmtCreate), Rep.Some(gmtUpdate)).shaped.<>({r=>import r._; _1.map(_=> TTradeOrderRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7.get, _8.get, _9.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))

    /** Database column trade_voucher_no SqlType(VARCHAR), PrimaryKey, Length(64,true) */
    val tradeVoucherNo: Rep[String] = column[String]("trade_voucher_no", O.PrimaryKey, O.Length(64,varying=true))
    ...
    /** Database column diamand_amount SqlType(INT) */
    val diamandAmount: Rep[Int] = column[Int]("diamand_amount")
    /** Database column amount SqlType(DECIMAL) */
    val amount: Rep[scala.math.BigDecimal] = column[scala.math.BigDecimal]("amount")
  ...
  /** Collection-like TableQuery object for table TTradeOrder */
  lazy val TTradeOrder = new TableQuery(tag => new TTradeOrder(tag))
}
